...
namespace SSDReportingusingDotNet
{
    ...
    public class RestServiceImpl : IRestServiceImpl
    {
        public string GetInventoryData(string SAPserialNumbers, string MAMserialNumbers)
        {
            ...
            if (!tds.connectionError)
            {
               ...
               // stringSQL = "select * from WW_BE_DM.SAP_BATCH_SERIAL WHERE SERIAL_NO \n IN (" + SAPserialNumbers + ");";
                ...
                stringSQL = stringSQL + "    AS\n";
                stringSQL = stringSQL + "    (\n";
                stringSQL = stringSQL + "       SELECT * FROM CTE_MA_ALL WHERE MA_ID NOT IN(SELECT MA_ID FROM CTE_MA_Rework)\n";
                stringSQL = stringSQL + "    ),\n";

                stringSQL = stringSQL + "   CTE_MA_Rework\n";
                stringSQL = stringSQL + "   AS(\n";
                stringSQL = stringSQL + "       SELECT MA_ID \n";
                stringSQL = stringSQL + "       FROM   WW_BE_DM.MA_attr\n";
                stringSQL = stringSQL + "       WHERE  MA_ID IN(SELECT MA_ID FROM CTE_MA_ALL)\n";
                stringSQL = stringSQL + "       AND   SYSTEM_NAME IN('MAMQA', 'MAMQASI')\n";
                stringSQL = stringSQL + "       AND   ATTR_ID = 'REWORK TYPE'\n";
                stringSQL = stringSQL + "       AND   ATTR_VALUE = 'COMPONENT REMOVAL'\n";
                stringSQL = stringSQL + "   ),\n";

                stringSQL = stringSQL + "    CTE_MA_ALL\n";
                stringSQL = stringSQL + "    AS\n";
                stringSQL = stringSQL + "    (\n";
        ...
        public string GetInventoryDatafromSingapore(string SAPserialNumbers, string MAMserialNumbers)
        {

            TeraDataRetriever tds = new TeraDataRetriever("SGTERAPROD09", "TERADATAREADER", "TeradataReader", "DataTera01");
            if (!tds.connectionError)
            {
                string stringSQL;
                // stringSQL = "select * from WW_BE_DM.SAP_BATCH_SERIAL WHERE SERIAL_NO \n IN (" + SAPserialNumbers + ");";
